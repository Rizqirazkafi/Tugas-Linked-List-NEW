#include <iostream>
using namespace std;
// Pendeklarasian dan inisialisasi linked list Mahasiswa (berurutan dari yang
// paling kecil (head) ke yang paling besar (tail))
struct Mahasiswa {
  string nama;
  long NIM;
  Mahasiswa *next;
...
void isi(string input_nama, long input_NIM) {
  ...
  // Jika linked list tidak kosong
    ...
    // Jika cur menunjuk ke node paling terakhir, taruh node baru di paling
    // belakang
    if (cur == NULL) {
      prev->next = baru;
      baru->next = NULL;
    ...
    // Jika cur tidak menunjuk ujung list, sisipi node baru di antara node-node
    // lain
    else {
      prev->next = baru;
      baru->next = cur;
...
void hapus(string input_nama, long input_NIM) {
  ...
  if (cur == NULL) {
    cout << endl;
    cout << "Maaf, data yang Anda inginkan untuk dihapus tidak ada" << endl;
  }
}
